...
| OmniRoute (веб-дашборд) | `Omni_2026!` | Хранится в его собственной sqlite, не в `.env` |
| Telegram | — (токен, не пароль) | Из @BotFather |

⚠️ Эти пароли **публичные** — они лежат в открытом репозитории. Годятся только для урока/демо на тестовом сервере. Каждый модуль после установки печатает точную команду, как сменить именно его пароль (раздел «⚠️» в конце вывода и в `~/<модуль>-credentials.txt`) — все команды смены собраны в одном месте и в **[СМЕНА_ПАРОЛЕЙ.md](СМЕНА_ПАРОЛЕЙ.md)**.
